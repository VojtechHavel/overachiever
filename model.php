 ...
 * this file has functions that communicate with database
 */
...
function increaseUsersPoints($amount)
{
    global $USER;
    global $DB;
    $UserId = $USER->id;
    ...
    $DB->update_record('block_oa_users', array('id' => $fieldId, 'points' => $points + $amount));
    return $amount;
}
...
function questionAnswered($params)
{
    ...
    $amount = increaseUsersPoints(ceil($params['fraction'] * (getCurrentStreak() + 1)));
    return array('pointsinc' => $amount);
}
...
function easyFirst($questions, $count)
{
    global $DB;
    $question_ids = [];
    $result = [];
...
function getQuestionStreak()
{
    ...
    if ($row = getCurrentQuestionId()) {
        ...
        $question_ids = easyFirst($questions, 6);
